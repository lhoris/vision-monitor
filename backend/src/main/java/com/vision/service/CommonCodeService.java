...
public class CommonCodeService {
    ...
    @Transactional
    public void activateCode(String actorUsername, Long id) {
        requireAdmin(actorUsername);
        Code code = findCode(id); code.setDataEndStatus(ACTIVE); codeRepository.save(code);
    }
    ...
    public CommonCodeDetailDto createDetail(String actorUsername, Long codeId, CommonCodeDetailDto request) {
        ...
        CodeDetail saved = detailRepository.save(CodeDetail.builder().codeId(codeId).value(value).name(resolveDisplayName(request))
                .nameKo(trimOrNull(request.getNameKo())).nameEn(trimOrNull(request.getNameEn()))
    ...
    public CommonCodeDetailDto updateDetail(String actorUsername, Long codeId, Long detailId, CommonCodeDetailDto request) {
        ...
        detail.setValue(value); detail.setName(resolveDisplayName(request)); detail.setNameKo(trimOrNull(request.getNameKo())); detail.setNameEn(trimOrNull(request.getNameEn())); detail.setDescription(trimOrNull(request.getDescription()));
    ...
    @Transactional
    public void activateDetail(String actorUsername, Long codeId, Long detailId) {
        requireAdmin(actorUsername); findCode(codeId);
        CodeDetail detail = detailRepository.findById(detailId).filter(value -> codeId.equals(value.getCodeId()))
                .orElseThrow(() -> new ApiException("CODE_DETAIL_NOT_FOUND", "공통코드 상세값을 찾을 수 없습니다."));
        if (detailRepository.existsByCodeIdAndValueIgnoreCaseAndDataEndStatus(codeId, detail.getValue(), ACTIVE)) throw new ApiException("DUPLICATE_CODE_VALUE", "이미 활성화된 코드값입니다.");
        detail.setDataEndStatus(ACTIVE); detailRepository.save(detail);
    }
    ...
    private RuntimeCommonCodeDto toRuntime(List<Code> codes) {
        ...
        codes.forEach(code -> result.put(code.getName().toUpperCase(Locale.ROOT), RuntimeCommonCodeDto.CommonCodeGroupDto.builder()
                ...
                .items(detailRepository.findAllByCodeIdAndDataEndStatusOrderBySortOrderAscIdAsc(code.getId(), ACTIVE).stream().map(detail -> RuntimeCommonCodeDto.CommonCodeItemDto.builder()
                        .id(detail.getId()).value(detail.getValue()).name(detail.getName()).nameKo(detail.getNameKo()).nameEn(detail.getNameEn()).description(detail.getDescription()).sortOrder(detail.getSortOrder()).defaultValue(detail.getDefaultValue()).build()).toList()).build()));
    ...
    private void validateDetail(CommonCodeDetailDto request) { if (request == null || request.getValue() == null || request.getValue().isBlank()) throw new ApiException("VALIDATION_ERROR", "코드값은 필수입니다."); if (resolveDisplayName(request).isBlank()) throw new ApiException("VALIDATION_ERROR", "코드값명은 필수입니다."); }
    private String resolveDisplayName(CommonCodeDetailDto request) {
        if (request == null) return "";
        if (request.getName() != null && !request.getName().isBlank()) return request.getName().trim();
        if (request.getNameKo() != null && !request.getNameKo().isBlank()) return request.getNameKo().trim();
        return trimOrNull(request.getNameEn()) == null ? "" : request.getNameEn().trim();
    }
