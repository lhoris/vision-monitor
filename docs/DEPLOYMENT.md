...
`scripts/deploy.bat`은 빌드나 설치 없이 복사된 배포 파일만 실행합니다. 인터넷 연결된 PC에서 `scripts\build.bat`을 실행한 뒤 저장소의 `backend\target` JAR, `frontend\dist`, 프로젝트 내부 Nginx 런타임(`nginx\windows\runtime`)과 스크립트를 함께 폐쇄망 PC로 복사하고 `scripts\deploy.bat`만 실행합니다. 배포 PC에는 Java 21이 설치되어 있어야 하며 `PROJECT_JAVA_HOME` 또는 `JAVA_HOME`으로 경로를 지정할 수 있습니다. 배포 스크립트는 Java, Maven, Node.js/npm을 다운로드하거나 설치하지 않습니다. Maven과 Node.js는 빌드 PC에서만 필요합니다. frontend 산출물과 정확히 하나의 backend JAR가 없으면 중단합니다. 포트 8080 점유 프로세스가 Vision Monitor JAR가 아니면 종료하지 않습니다. backend 준비 후 프로젝트 내부 Nginx 실행 파일·설정·정적 root 경로를 출력하고, `/`, 빌드된 JS/CSS asset, `/api/common-codes/bootstrap` 프록시 응답까지 확인합니다. `/api` 경로는 backend로 전달하고 나머지는 `deploy\www`의 정적 파일과 SPA 경로로 처리합니다. 정적 asset 파일이 없으면 HTML 대신 404를 반환합니다.
