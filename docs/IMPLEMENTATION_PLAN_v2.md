...
- **개인화 가능한 그리드 대시보드** (공정별 탭, 사용자 정의 레이아웃, 드래그&드롭, 동적 CCTV 선택) ⭐ **핵심 요구사항**
...
Layouts (id, user_id, tab_name, grid_config JSON, camera_positions JSON, created_at, updated_at)  -- 개인화 그리드 저장
```
...
2. **Architecture & Design Agent** (1명)
   - 시스템 전체 설계
   - REST API 스펙 정의 (개인화 그리드 API 포함: /api/layouts)
   - MariaDB 스키마 설계
   - 설계서 기반 화면 구성 정리
   - 배포 전략 수립
   - 결과: `ARCHITECTURE.md`, `API.md`, `SCREENS.md`

### Phase 2: 하네스 엔지니어링 (1주)

3. **Harness Engineer** (1명)
   - Spring Boot 보일러플레이트
   - React + Vite 보일러플레이트 (React Dnd 포함)
   - 배포 쉘 스크립트 기본 구조
   - 개발 환경 가이드
   - 로컬 개발 서버 세팅
   - 결과: 실행 가능한 프로젝트 구조
...
6. **VMS Core Pages & Grid Personalization Agent** (1명) ⭐ **높은 복잡도**
   - 담당: 주요 페이지 & **개인화 그리드 핵심 기능**
   - 구현:
     - **라이브 모니터링 페이지** (개인화 그리드: 탭, 레이아웃 선택, D&D, 동적 CCTV 선택)
     - 단일 카메라 상세 뷰
     - 타임라인 기반 재생 페이지
     - 카메라 설정 페이지
     - 이벤트/알림 관리
     - 설정 페이지
   - 파일 소유: `frontend/src/pages/**`, `frontend/src/components/{Grid,CameraDetail}/**`
   - 복잡도: **높음** (개인화 그리드 D&D 구현)
   - 의존성: React Dnd, State Management Agent의 Layout 스토어

7. **State Management & API Integration Agent** (1명)
   - 담당: 전역 상태 & REST API
   - 구현:
     - Redux/Zustand 스토어 (Layout 스토어 포함)
     - API 클라이언트 (REST)
     - 커스텀 훅 (useCamera, useStream, useTimeline, useEvents, **useLayout**)
     - 에러 처리 & 로딩 상태
...
8. **Spring Boot Core & Database Agent** (1명)
   ...
   - 구현:
     - Spring Boot 프로젝트 세팅
     - MariaDB 커넥션 풀
     - JPA 엔티티 설계 (Camera, Stream, Recording, Event, AlertSetting, **Layout**)
     - Repository 계층
     - 마이그레이션 스크립트 (Flyway)
...
9. **REST API & Controller Agent** (1명)
   ...
   - 구현:
     ...
     - Event/Alert 조회 API
     - 녹화 관리 API
     - **Layout API** (GET/POST/PUT /api/layouts, /api/layouts/:userId)
     - 실시간 이벤트 푸시 (SSE)
...
10. **Business Logic & Middleware Agent** (1명) - **높은 복잡도**
    - 담당: 서비스 로직 & L2/EAI 연동
    - 구현:
      - 서비스 계층 (CameraService, EventService, RecordingService, **LayoutService**)
      - 이벤트 처리 & 알림 시스템
...
VMS Core Pages & Grid Personalization Agent:
├── frontend/src/pages/**
├── frontend/src/components/Grid/**
└── frontend/src/components/{CameraDetail,Events,Settings}/**
...
### Week 1-2: 리서치 & 설계
- [ ] Research Agent: 기업용 VMS 스타일 분석 (개인화 그리드 UI 패턴 포함)
- [ ] Architecture Agent: API 스펙 & 스키마 설계 완료 (/api/layouts 포함)
- [ ] Harness Engineer: 프로젝트 보일러플레이트 (React Dnd 포함)

### Week 3: 기초 구현
- [ ] Spring Boot Core: 엔티티 & Repository (Layout 테이블 포함)
- [ ] State Management: 스토어 & 훅 골격 (Layout 스토어, useLayout 훅)
- [ ] **StreamPlayer & Grid: 추상화 클래스, HLSPlayer, 개인화 그리드 컴포넌트**
- [ ] UI Layout: 기본 레이아웃, 스타일 시스템, 공정별 탭 UI

### Week 4-5: 핵심 기능
- [ ] **VMS Core Pages: 라이브 뷰 (개인화 그리드, D&D), 상세 뷰**
- [ ] REST API: Camera, Stream, Event, **Layout** 엔드포인트
- [ ] StreamPlayer: WebRTCPlayer 구현
- [ ] Business Logic: 서비스 계층 기본 구현 (LayoutService 포함)

### Week 6: 고급 기능 & 통합
- [ ] Middleware: L2/EAI 연동 구현
- [ ] 타임라인: 재생 기능
- [ ] 실시간 업데이트: SSE/WebSocket
- [ ] 전체 E2E 테스트 (개인화 그리드 포함)
...
### 일일 동기화 (9:00 AM)
- 각 Agent 진행 상황 공유
- 블로커 해결 (API 스펙, 타입 정의, Layout 스토어 등)
- 파일 소유권 충돌 확인

### API 스펙 공유
- 초기 REST API 스펙 확정 (Week 1 마지막) - **Layout API 포함**
- Mock API 제공 (병렬 구현용)
- 변경사항 24시간 전 공지
...
- ✅ **개인화 그리드 대시보드** (공정별 탭, 사용자 정의 레이아웃, D&D, 동적 CCTV 선택) ⭐ **고객사 핵심 기능**
  - 비즈니스 근거: "나중에 비용 지불 재개발 방지" → 초기 구현에 필수 포함
- ✅ 타임라인 기반 재생
- ✅ 실시간 이벤트 알림
- ✅ 카메라 설정 관리
- ✅ WebRTC 스트림 수신 (주소 제공)
...
1. **라이브 모니터링** - 개인화 가능한 다중 카메라 그리드 뷰 ⭐ **핵심 기능**
   ...
   - **사용자 정의 그리드 레이아웃**: 3x2, 3x3, 2x3 등 자유 선택 (드롭다운)
   - **동적 카메라 배치**: 각 셀에 "+" 버튼 → 모달로 CCTV 목록 선택 가능
   - **드래그 & 드롭**: StreamPlayer를 다른 셀로 자유롭게 이동 (React Dnd)
   - **개인화 저장**: 사용자별 레이아웃 자동 저장 & 다음 접속 시 복원
   - 실시간 이벤트 인디케이터
   - 카메라 이름, 상태 표시
...
- ✅ 일일 통합 빌드 성공
- ✅ E2E 기능 테스트 통과 (주간 고객 데모) - **개인화 그리드 포함**
- ✅ 코드 커버리지 >80%
- ✅ API 응답 시간 <200ms
- ✅ 스트림 재생 지연 <2초
- ✅ **개인화 그리드 D&D 반응속도 <100ms**
...
1. **이 문서 검토 & 피드백**
2. **설계서 기반 화면 구성 상세화** (개인화 그리드 UI/UX 명시)
3. **Architecture Agent: 초기 설계 (API 스펙, DB 스키마, Layout 스토어 설계)**
4. **Harness Engineer: 보일러플레이트 생성 (React Dnd 통합)**
