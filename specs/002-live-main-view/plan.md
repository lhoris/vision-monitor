...
라이브 메인 화면은 기존 React frontend의 `Live` page, Grid 컴포넌트, layout store, mock camera/layout fixture를 기준으로 정리한다. MVP는 Spring Boot 실제 API 없이 frontend mock fallback과 Redux state를 사용하며, layout persistence API는 후속 backend 연동을 위한 계약 형태만 유지한다. 공정탭/세부공정탭/그리드/카메라 타일 조작은 현재 구현된 동작을 Spec Kit 기준 산출물로 승격하고, Add Camera에는 카메라 목록 선택 모드와 직접 영상 주소 입력 모드를 함께 제공한다. 직접 입력 모드는 WebRTC, RTSP, HLS 유형과 URL을 받아 현재 세부공정탭에 임시 영상 타일을 추가하며, 실제 media distribution과 장기 저장은 후속 범위로 둔다. 003 화면 확대 보기는 이 기능의 하위 구현이 아니라, 선택 카메라와 현재 세부공정탭 카메라 목록을 전달받는 별도 기능으로 연결한다.
...
| FR-003 | 5. 상태 및 상호작용 흐름, tasks US4 | 탭 추가/삭제/정렬 |
...
| FR-009 | 5. 상태 및 상호작용 흐름, tasks US4 | Remove |
| FR-010 | 5. 상태 및 상호작용 흐름, tasks US4 | Rename |
| FR-011 | 6. 데이터 및 계약 계획, tasks US4 | title override |
| FR-012 | 5. 화면/컴포넌트 구조, tasks US1 | video-safe header |
| FR-013 | 5. 화면/컴포넌트 구조, tasks US1 | status dot only |
| FR-014 | 5. 화면/컴포넌트 구조, tasks US5 | hover/focus action |
| FR-015 | 6. 계약, tasks US5 | focus route contract |
| FR-016 | 7. 테스트 및 검증 계획 | empty/fallback state |
| FR-017 | 7. 테스트 및 검증 계획 | theme contrast |
| FR-018, FR-019 | 5. 화면/컴포넌트 구조, tasks US3 | Add Camera 모드 토글과 WebRTC/RTSP/HLS 선택 |
| FR-020, FR-023 | 6. 데이터 및 계약 계획, tasks US3 | URL 검증과 동일 세부탭 중복 방지 |
| FR-021, FR-022, FR-025 | 5. 화면/컴포넌트 구조, tasks US3 | 임시 영상 타일과 표시 제목 |
| FR-024 | 7. 테스트 및 검증 계획, tasks US3 | 연결/재생/실패 상태와 수정·제거 |

## 3. 기술 컨텍스트

**언어/버전**: TypeScript, React 19, Vite 8

**주요 의존성**: React Router, Redux Toolkit, Vitest, React Testing Library, hls.js, video.js. WebRTC는 브라우저의 표준 재생 경계를 사용한다.
...
**규모/범위**: 공정탭, 세부공정탭, 2x3/3x3/3x2/2x4/4x2/4x4 그리드, 카메라 7개 mock fixture, 직접 영상 주소 임시 타일, 003 route 진입
...
- `frontend/src/components/Grid/`의 Add Camera 모드 토글, 직접 영상 주소 입력 dialog, 프로토콜 선택, URL 검증, 임시 영상 타일
...
- `frontend/src/components/StreamPlayer/LiveStreamPlayer.tsx`의 WebRTC/RTSP/HLS 입력 상태와 연결·재생·실패 상태 표시
...
- `Add Camera`는 카메라 목록 모드와 직접 영상 주소 모드를 토글로 제공하며, 직접 입력 모드의 기본 프로토콜은 WebRTC로 둔다.
- 직접 영상 주소 모드는 주소 유형, URL, 선택적 표시 제목을 입력받고, 입력칸의 텍스트/caret/focus 상태를 테마와 무관하게 명확히 표시하며, 유효성 검증을 통과한 값을 임시 영상 타일로 전달한다.
- 직접 추가 영상은 카메라 목록의 `Camera`와 별도의 임시 source로 관리하며, 같은 세부공정탭의 동일 URL 중복을 차단한다.
- 재생기는 연결 중·재생 중·재생 실패 상태를 타일 단위로 표시하고, 실패 타일은 주소 수정 또는 제거가 가능해야 한다.
...
- 직접 영상 주소는 MVP에서 현재 세부공정탭의 임시 배치로만 관리하며, 카메라 목록이나 backend layout 원본을 변경하지 않는다.
...
- 직접 추가 영상은 `sourceType=temporary`, `streamProtocol`, `streamUrl`, `displayName`을 가진 source로 정규화한 뒤 동일한 타일 렌더링 경로를 사용한다.
...
- TemporaryVideoSource: sourceType, protocol, url, displayName, playbackStatus
- Add Camera mode and source validation result
...
- WebRTC/RTSP/HLS 유형별 유효 URL, 잘못된 URL, 동일 세부탭 중복 URL, 재생 실패 temporary source fixture

## 7. 테스트 및 검증 계획

- **단위/컴포넌트 테스트**: `DraggableCell`, `GridContainer`, `TabsBar`, `SubTabsBar`, `LayoutSelector`, `CameraSelector`, `AddCameraDialog`, `TemporaryVideoTile`
- **서비스/계약 테스트**: `layoutService`, `liveMonitoring` fixture, Add Camera source validation, temporary source normalization, route query 생성
...
| 브라우저가 선택한 WebRTC/RTSP/HLS 주소를 재생하지 못함 | 직접 추가 영상이 빈 타일로 보임 | protocol별 연결/지원 불가/재생 실패 상태를 구분하고 URL 수정·제거를 제공 |
| 직접 영상 주소가 기존 카메라 목록 또는 다른 임시 타일과 중복됨 | 동일 영상이 중복 배치됨 | 현재 세부공정탭 기준 URL 중복 검증 |
...
| 직접 영상 주소를 temporary source로 분리 | 카메라 목록과 임시 영상의 책임 경계가 다름 | MVP 장기 저장 없이 현재 세부공정탭에서만 관리 |
