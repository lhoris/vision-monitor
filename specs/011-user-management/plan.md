...
이 섹션은 기존 계획의 상세 패널 중심 문구보다 우선한다. 기존에 구현된 `UserDetailPanel`과 `UserEditDialog` 흐름은 구현 이력으로 유지하되, 이후 구현 기준은 전체 폭 그리드, 행 추가, 셀 인라인 편집, 선택 행 batch action이다.

### 설계 결정

사용자 관리 화면은 우측 상세 패널을 기본 구조에서 제거하고, 전체 폭 Tabulator 그리드를 주 작업 영역으로 삼는다. 신규 사용자 생성, 기존 사용자 정보 수정, 계정 폐기 업무는 그리드의 행 추가, 셀 편집, 선택 행 작업으로 처리한다.
...
│ - 행 선택 및 일괄 작업                                       │
│ - 신규/수정/폐기요청/오류 상태 표시                          │
│ - 컬럼 표시/숨김, 순서, 너비, 정렬, 필터                      │
└──────────────────────────────────────────────────────────────┘
```

### 컴포넌트 조정

- `UserManagementGrid`: 목록 표시뿐 아니라 행 추가, 인라인 편집, 다중 선택, batch action, 변경 상태 표시, 셀 오류 표시를 담당한다.
- `UserDetailPanel`: 기본 화면에서 제거한다. 필요한 상세 정보는 `UserDetailDialog` 또는 grid row expansion으로 대체한다.
- `UserEditDialog`: 신규/수정의 기본 경로에서 제외한다. 복잡한 확인이 필요한 경우의 보조 modal로 축소한다.
- `UserDangerDialog`: 단건 위험 작업 보조 흐름으로 유지한다. 그리드 선택 행 batch action은 그리드 toolbar의 확인 흐름을 사용한다.
- `UserStatusBadge`: 그리드 셀 formatter에서 계속 사용한다.

### 기존 계획 대체 매핑

| 기존 흐름 | 개정 후 기준 |
|-----------|--------------|
| 행 선택 시 우측 상세 패널 동기화 | 행 선택은 선택 상태와 batch action 대상으로만 사용 |
| 신규/수정 dialog 중심 입력 | 그리드 행 추가와 셀 인라인 편집을 기본 입력으로 사용 |
| 목록과 상세 snapshot 동기화 | 그리드 row state와 원본 snapshot 동기화로 대체 |
| 상세 패널의 역할/개인화 정보 표시 | 필요 시 row expansion 또는 modal에서만 조회 |
| 단건 상태 변경 중심 UI | 선택 행 batch action toolbar 중심 UI |

### 상태 모델 조정

- `draftRows`: 원본 사용자 목록 위에 적용되는 편집 중 행 상태
- `selectedRowIds`: 일괄 작업 대상 행
- `dirtyRowIds`: 저장이 필요한 수정 행
- `newRowIds`: 아직 저장되지 않은 신규 행
- `discardRequestedRowIds`: 폐기 요청 대상 행
- `rowErrors`: 행/필드별 validation 오류

### 동작 원칙

- 행 클릭은 상세 패널 열기가 아니라 행 선택 또는 셀 편집에 집중한다.
- 신규 등록은 빈 행 추가 후 셀 편집으로 처리한다.
- 저장은 변경된 행만 대상으로 수행하되, 화면에는 전체 그리드의 변경 상태를 표시한다.
- 저장 실패 시 draft 상태를 보존한다.
- 자기 자신에 대한 삭제, 폐기, 비활성화, 잠금은 차단한다.

### 검증 계획

- 행 추가 후 필수값 오류가 셀 단위로 표시되는지 검증한다.
- 인라인 편집 후 저장 전 변경 상태가 표시되는지 검증한다.
- 여러 행 선택 후 잠금, 비활성화, 퇴사, 폐기 요청이 그리드 상태로 반영되는지 검증한다.
- 자기 자신에 대한 폐기/비활성화/잠금 작업이 차단되는지 검증한다.
- 우측 상세 패널 없이도 사용자 조회, 생성, 수정, 폐기 업무가 완료되는지 검증한다.
