...
### 3. tester1 비관리자 mock 로그인

1. username에 `tester1`, password에 `tester123`을 입력한다.
2. Sign In을 선택한다.
3. 로그인 후 사이드바를 연다.

**기대 결과**: 로그인 API 호출 없이 `/live`로 이동하고, 관리자 메뉴는 표시되지 않는다.

### 4. tester1 잘못된 비밀번호

1. username에 `tester1`, password에 다른 값을 입력한다.
2. Sign In을 선택한다.

**기대 결과**: 로그인 API 호출 없이 오류가 표시된다.

### 5. non-mock 계정 API 호출
