...
import type { MetadataChartMapping, MetadataGridMapping, MetadataQueryDefinition, MetadataSectionConfig, MetadataSectionType, MetadataTextMapping } from '@/types/metadataConfig'
...
export function MetadataSectionEditor({ section, queries, onSave, onClose }: MetadataSectionEditorProps) {
  ...
  function submit(event: FormEvent) {
    ...
    if (type === 'text') mapping = { ...(labelField ? { labelField } : {}), ...(valueField ? { valueField } : {}) }
    ...
    onSave({ id: section?.id ?? `section-${Date.now()}`, title: title.trim(), type, order: section?.order ?? 99, visible: true, queryId: queryId || undefined, refreshIntervalSec: interval, mapping, defaultText: type === 'text' ? defaultText : undefined, sourceProfileStatus: 'included' })
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <form onSubmit={submit} className="w-full max-w-md space-y-4 border bg-white p-5 shadow-xl dark:border-slate-700 dark:bg-slate-800">
        ...
        {type === 'text' ? <><label className="block text-sm">기본 텍스트<textarea value={defaultText} onChange={(event) => setDefaultText(event.target.value)} rows={3} placeholder="조회 결과가 없을 때 표시할 안내 문구" className="mt-1 w-full resize-y border px-3 py-2 dark:bg-slate-900" /></label>{query ? <div className="grid grid-cols-2 gap-2"><FieldSelect label="라벨 필드" value={labelField} fields={fields} onChange={setLabelField} allowEmpty /><FieldSelect label="값 필드" value={valueField} fields={fields} onChange={setValueField} allowEmpty /></div> : null}</> : null}
