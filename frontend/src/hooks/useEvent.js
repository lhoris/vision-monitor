...
import { fetchEvents, fetchCameraEvents, acknowledgeEventAsync, acknowledgeEventsAsync, deleteEventAsync, fetchAlertSettings, createAlertSettingAsync, updateAlertSettingAsync, deleteAlertSettingAsync, setEventFilter, addEvent, updateEvent, acknowledgeEvent, clearError, } from '@/store/slices/eventSlice';
export function useEvent() {
     ...
     * 카메라별 이벤트 조회
     */
    const loadCameraEvents = useCallback((cameraId, params) => {
        dispatch(fetchCameraEvents({ cameraId, params }));
    }, [dispatch]);
    /**
     * 단일 이벤트 확인 처리
     */
    const acknowledgeEventFn = useCallback((eventId) => {
        return dispatch(acknowledgeEventAsync(eventId));
    }, [dispatch]);
    /**
     * 여러 이벤트 확인 처리
     */
    const acknowledgeEventsFn = useCallback((eventIds) => {
        return dispatch(acknowledgeEventsAsync(eventIds));
    }, [dispatch]);
    /**
     * 이벤트 삭제
     */
    const deleteEventFn = useCallback((eventId) => {
        return dispatch(deleteEventAsync(eventId));
    }, [dispatch]);
    /**
     * 알림 설정 조회
     */
    const loadAlertSettings = useCallback((cameraId) => {
        dispatch(fetchAlertSettings(cameraId));
    }, [dispatch]);
    /**
     * 알림 설정 생성
     */
    const createAlertSetting = useCallback((setting) => {
        return dispatch(createAlertSettingAsync(setting));
    }, [dispatch]);
    /**
     * 알림 설정 업데이트
     */
    const updateAlertSetting = useCallback((id, setting) => {
        return dispatch(updateAlertSettingAsync({ id, setting }));
    }, [dispatch]);
    /**
     * 알림 설정 삭제
     */
    const deleteAlertSetting = useCallback((id) => {
        return dispatch(deleteAlertSettingAsync(id));
    }, [dispatch]);
    /**
     * 필터 설정 변경
     */
    const onSetFilter = useCallback((filterData) => {
        dispatch(setEventFilter(filterData));
    }, [dispatch]);
    /**
     * 로컬 이벤트 추가 (웹소켓 등을 통해)
     */
    const onAddEventLocal = useCallback((event) => {
        dispatch(addEvent(event));
    }, [dispatch]);
    /**
     * 로컬 이벤트 업데이트
     */
    const onUpdateEventLocal = useCallback((event) => {
        dispatch(updateEvent(event));
    }, [dispatch]);
    /**
     * 로컬 이벤트 확인
     */
    const onAcknowledgeEventLocal = useCallback((eventId) => {
