...
export function VideoManagement() {
  ...
  const load = useCallback(async () => {
    setError('')
    try {
      setItems(await videoSourceService.list())
  ...
  const openEdit = (item: VideoSource) => {
  ...
  const save = async (value: VideoSourceInput) => {
    ...
    setModalError('')
    try {
      const saved = editingId === null ? await videoSourceService.create(value) : await videoSourceService.update(editingId, value)
      if (!saved) throw new Error('영상 주소 저장에 실패했습니다.')
      setModalOpen(false)
      setNotice(editingId === null ? '영상 주소를 추가했습니다.' : '영상 주소를 수정했습니다.')
      await load()
    } catch (exception) {
  ...
  return (
    <section className="flex h-full min-h-0 flex-col gap-4 bg-slate-50 p-4 dark:bg-slate-950 md:p-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <button type="button" onClick={() => openAdd()} className="rounded bg-blue-600 px-3 py-2 text-sm font-semibold text-white hover:bg-blue-700">영상 추가</button>
      </div>
      ...
      <section className="relative flex min-h-0 flex-1 flex-col border border-[#9dcced] bg-white shadow-none dark:border-slate-700 dark:bg-slate-800">
        <div className="flex flex-wrap items-center gap-2 border-b border-slate-200 px-3 py-2 dark:border-slate-700">
          <input value={query} onChange={(event) => setQuery(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') setAppliedQuery(query) }} placeholder="영상 이름 또는 주소 검색" className="h-8 w-64 rounded border border-slate-300 px-2 text-sm text-slate-900 dark:border-slate-600 dark:bg-slate-900 dark:text-white" />
          <button type="button" onClick={() => setAppliedQuery(query)} className="rounded border border-slate-300 px-3 py-1.5 text-sm dark:border-slate-600">검색</button>
          <button type="button" onClick={() => openAdd()} className="rounded border border-slate-300 px-3 py-1.5 text-sm dark:border-slate-600">추가</button>
          <button type="button" onClick={duplicateSelected} disabled={selectedIds.size !== 1} className="rounded border border-slate-300 px-3 py-1.5 text-sm disabled:opacity-50 dark:border-slate-600">복제</button>
          <button type="button" onClick={() => void deleteSelected()} disabled={!selectedIds.size || saving} className="rounded border border-rose-300 px-3 py-1.5 text-sm text-rose-700 disabled:opacity-50 dark:border-rose-700 dark:text-rose-300">삭제</button>
          <button type="button" onClick={() => setFiltersVisible((current) => !current)} className="rounded border border-slate-300 px-3 py-1.5 text-sm dark:border-slate-600">필터</button>
          <button type="button" onClick={() => void load()} disabled={saving} className="rounded border border-slate-300 px-3 py-1.5 text-sm disabled:opacity-50 dark:border-slate-600">새로고침</button>
        </div>
          ...
          <table className="w-full min-w-[1120px] table-fixed border-separate border-spacing-0 border-l border-t border-[#9dcced] text-left text-sm dark:border-slate-700">
            <thead className="sticky top-0 z-10">
              <tr>
                ...
                <th className={`w-24 ${headerCell}`}>작업</th>
              </tr>
            ...
            <tbody>
              {visibleItems.map((item) => (
                <tr key={item.id} className={`${selectedIds.has(item.id) ? 'bg-[#d6edff] dark:bg-sky-950/40' : 'bg-white dark:bg-slate-800'} h-9 text-slate-800 hover:bg-[#eef8ff] dark:text-slate-100 dark:hover:bg-slate-700`}>
                  ...
                  <td className={`${bodyCell} truncate font-medium`}>{item.name}</td>
                  <td className={`${bodyCell} truncate font-mono text-xs`} title={item.url}>{item.url}</td>
                  <td className={bodyCell}>{item.protocol}</td>
                  <td className={bodyCell}>{item.location || '-'}</td>
                  <td className={bodyCell}>{item.zone || '-'}</td>
                  <td className={bodyCell}><span className={item.status === 'ACTIVE' ? 'text-emerald-600' : 'text-slate-500'}>{item.status}</span></td>
                  <td className={bodyCell}><button type="button" onClick={() => openEdit(item)} className="text-blue-600 hover:underline">수정</button></td>
                </tr>
              ))}
              {!visibleItems.length ? <tr><td colSpan={8} className="p-10 text-center text-sm text-slate-500">조건에 맞는 영상 주소가 없습니다.</td></tr> : null}
            </tbody>
          </table>
        </div>
      ...
      <VideoSourceModal open={modalOpen} initialValue={modalValue} title={editingId === null ? '영상 추가' : '영상 수정'} saving={saving} error={modalError} onClose={() => setModalOpen(false)} onSubmit={save} />
    </section>
  )
}
